 ...
 *                       08 jul 99 : traitement des buffer EBYEDAT (exogam)  *
 *                      				                     *
 *                                                                           *
  ...
  a quelqu'un d'autre.		*/
	...
	while ( Buffer_suivant ) {
	  ...
	  if ( Status == ACQ_OK ) {
	      ...
	      if ( strcmp ( Header, PARAM_Id) == 0 ) {
		 /*	
                  * Buffer de liste de parametres, utilisation libre.
                  */               
	         puts ("\n>>> Buffer de parametres");
                 /* Afficher parametres si 2eme argument = TRUE */
		 Status = traite_params ( &Buffer, TRUE );

		 if ( Status != ACQ_OK )
		    gan_tape_erreur ( Status, "traite parametres");
	       }

	      else if ( strcmp ( Header , SCALER_Id ) == 0 ) {
		  /*	
	           * Buffer echelle, utilisation libre.
                   */               
		  puts ("\n>>> Buffer echelle");
	      	  Status = traite_echelles ( &Buffer ); /* Traitement. */
		  if ( Status != ACQ_OK )
		    gan_tape_erreur ( Status, "traite echelles");
  
		}

	      else if ( strcmp ( Header , EVENTDB_Id ) == 0 ) {
		  /* 
		   * Traitement bloc d'evenements bruts 
		   */

                  Event_suivant = true;
		  EventNumber = -1;
		  while ( Event_suivant ) {
		     Status = get_next_event ( &Buffer, BufSize, EventBrut, 
					       Evbsize, &EventNumber );
		     if ( Status == ACQ_OK ) {
			Status = s_evctrl ( EventBrut, EventCtrl,
					    Struct_event, &CtrlForm );
                        if ( Status == GR_OK ){
			   /*
			    *  Traitement de l'evenement.
			    */
			   Status = mon_traitement(EventCtrl);
			   if ( Status != ACQ_OK )
			      puts ("\n>>> Erreur dans mon traitement");
			 }
			 else puts ("\n>>> Erreur de reconstruction");
		      }
		      else if ( Status == ACQ_ENDOFBUFFER )
			 Event_suivant = false;
		      else {
			 gan_tape_erreur (Status,"obtention d'evenement");
			 Event_suivant = false;
			 Buffer_suivant = false;
		       }
		   } /* fin du while Event_suivant */
		} /* fin traitement bloc EVENTDB */

	      else if ( strcmp ( Header , EVENTCT_Id ) == 0 ) {
		  /* 
		   * Traitement bloc d'evenements de control 
		   */

                  Event_suivant = true;
		  EventNumber = -1;
		  while ( Event_suivant ) {
		     Status = get_next_event ( &Buffer, BufSize, EventCtrl, 
					       Evcsize, &EventNumber );
		     if ( Status == ACQ_OK ) {
			   /*
			    *  Traitement de l'evenement.
	      ...
	      else if ( strcmp ( Header , EBYEDAT_Id ) == 0 ) {
		  /* 
		   * Traitement bloc d'evenements EBYEDAT
		   *
		   *  sequence developpee pour la mise au point de 
		   *  acq_ebyedat_get_next_event 
		   * On utilise acq_ebyedat_get_next_event_r qui est le 
		   * point d'entree rentrant de acq_ebyedat_get_next_event
		   * Ceci permet d'appeler 2 fois la fonction pour obtenir
		   * le meme evenement dans deux formats differents :
		   *  - Le premier appel retourne un evenement au format de 
		   *    control variable GANIL, puis on appelle mon_traitement
		   *    pour affectuer une statistique sur les paramatres
		   *    et dumper un evenement sur 0x10000
		   * -  Le deuxieme appel retourne un pointeur sur le meme 
		   *    evenement sans transformation de format et l'appel
		   *    a mon_traitement_exogam dump si mon_traitement a dumpe
		   */
		UNSINT16 *MyEvtCtrl, *MyNextCtrl;
		UNSINT16 *MyRawEvt, *MyNextRaw;
		int RawEventNumber = -1;

                  Event_suivant = true;
		  EventNumber = -1;
		  while ( Event_suivant ) {
		    /* get next event and return it in GANIL 
		       variable control event format */
		     Status = acq_ebyedat_get_next_event_r ( (UNSINT16 *)&Buffer, &MyEvtCtrl, 
					       &EventNumber, EVCT_VAR, &MyNextCtrl );
		     if ( Status == ACQ_OK ) {
			   /*
			    *  Traitement de l'evenement.
			    */
			   Status = mon_traitement((short int *)MyEvtCtrl);
			   if ( Status != ACQ_OK )
			      puts ("\n>>> Erreur dans mon traitement");

			   /* Get Raw Event */
			   else {
			     Status = acq_ebyedat_get_next_event_r ( (UNSINT16 *)&Buffer, 
								     &MyRawEvt, 
					       &RawEventNumber, -1, &MyNextRaw );
			     if ( Status == ACQ_OK ) {
			       Status = mon_traitement_exogam(MyRawEvt);
			       if ( Status != ACQ_OK )
				 puts ("\n>>> Erreur dans mon traitement_exogam");
			     }
			   }
		     }
		      else if ( Status == ACQ_ENDOFBUFFER )
	...
	/* Afficher les statistiques de mon traitement */
	AfficheStat();
