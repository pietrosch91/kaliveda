...
EnergyTree::EnergyTree(LogFile *Log,Sive503 *SiD)
{
  ...
  Si = SiD;
    ...
    fpTelescope = 0;
    //fpTelescope = new KVTelescope();

    fpCsI = 0;
    //fpCsI = new KVCsIVamos();

    fpSi = 0;
    //fpSi = new KVSiliconVamos();
...
void EnergyTree::InitTelescope(Int_t si_num ,Int_t csi_num)
{
    ...
    L->Log << "EnergyTree::InitTelescope(" << si_num << "," << csi_num << ")" << endl;
    ...
    if(list != 0){
        ...
        L->Log << " * Error: kvid assignment failed (SI_" << si_num << "_CSI_" << csi_num << ")" << endl;
...
void EnergyTree::InitSiCsI(Int_t number) // Si-CsI Telescope	# Si : 1 to 18	
{
    ...
    L->Log << "fpTelescope->GetName(): " <<  fpTelescope->GetName() << endl;
    ...
    fpSi = (KVSiliconVamos*) fpTelescope->GetDetector((UInt_t)1); 
    fpGap = (KVDetector*) fpTelescope->GetDetector((UInt_t)2);
    fpCsI = (KVCsIVamos*) fpTelescope->GetDetector((UInt_t)3);

    lum = new KVLightEnergyCsIVamos(fpCsI);
}
...
void EnergyTree::CalculateCanalCsI(){
      ...
      CanalCsI=lum->Invert(fpCsI->GetEnergy());
...
void EnergyTree::SimulateEvent(){
  ...
  //kvt_sicsi->DetectParticle(&part);
    fpTelescope->DetectParticle(&part);
...
void EnergyTree::PassTelescope(KVTelescope *telescope){   
    fpTelescope = telescope;
}
