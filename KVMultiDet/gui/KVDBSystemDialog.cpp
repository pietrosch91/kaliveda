...
void KVDBSystemDialog::UpdateProjectileProperties()
{
   ...
   if(!fSystem){
      fCheckButton1376->SetState(kButtonUp);EnableProjectileProperties(kFALSE); return;
   }
   KVNucleus proj(fSystem->GetZproj(), fSystem->GetAproj());
   //if no projectile is defined, the entire "Projectile" part is "greyed out"
   if(proj.GetZ()==0){
      fCheckButton1376->SetState(kButtonUp);EnableProjectileProperties(kFALSE);
   } else{
...
void KVDBSystemDialog::UpdateTargetProperties()
{
   ...
   if(!fTarget){
      fNumberEntry1532->SetNumber(0);UpdateTargetLayerProperties(-1);
      return;
   }
   //update target angle
   fNumberEntry1532->SetNumber(fTarget->GetAngleToBeam());
   Int_t index = 0;
   TIter next(fTarget->GetLayers()); TObject* obj;
   while( (obj = next()) ){
...
void KVDBSystemDialog::UpdateTargetLayerProperties(Int_t ind)
{
   ...
   if(ind==-1){
      ...
      fLayer=0;fTextButton1554->SetEnabled(kFALSE);
      return;
   }
...
void KVDBSystemDialog::ProjectileSymbolChanged()
{
   ...
   if(fSystem){
      fSystem->SetZbeam(r.GetZ()); fSystem->SetAbeam(r.GetA());
   }
   SetNeedSave(1);
}
...
void KVDBSystemDialog::RemoveTargetLayer()
{
   ...
   if(fTarget->NumberOfLayers() == 1){
      ...
      delete fTarget; fTarget = 0;
      if(fSystem) fSystem->SetTarget(new_target); fLayer = 0;
   }
   UpdateTargetProperties();
   SetNeedSave(1);
}
                             ...
                             UInt_t w, UInt_t h)
{
   ...
   fTextButton1573->Resize(48,24); fTextButton1573->ChangeBackground(red);
