...
class KVDataAnalyser : public TObject {
 ...
 public:
   ...
   virtual void SetBatchMode(Bool_t on=kTRUE) { fBatch = on; };
   virtual Bool_t BatchMode() const { return fBatch; };
   virtual void SetBatchName(const Char_t* batchname) { fBatchName = batchname; };
   virtual const Char_t* GetBatchName() { return fBatchName.Data(); };
   virtual void SetBatchSystem(KVBatchSystem* bs) { fBatchSystem = bs; fChoseRunMode = kTRUE; if(bs) fBatchSystem->cd();};
   virtual const KVBatchSystem* GetBatchSystem() { return fBatchSystem; };
   ...
   virtual void ClearRunList() { fRunList.Clear();};
   virtual void Reset();

   virtual void SetParent(KVDataAnalyser* da) { fParent = da; };
   virtual KVDataAnalyser* GetParent() const  { return fParent; };
   ...
   virtual void AddHistoToServer(TH1*, const Char_t* /* family */ = 0)
	{
		// DUMMY METHOD!!!
	};
