...
void KVDBSystem::GetRunList(KVNumberList& list) const
{
   ...
   list.Clear();
   TIter next( GetRuns() ); KVDBRun* run;
...
void KVDBSystem::Save(ostream &f) const
{
   ...
   if( fTarget ){
      ...
      TIter next(fTarget->GetLayers()); KVMaterial* lay;
...
void KVDBSystem::Load(istream &f)
{
   ...
   fAbeam=fZbeam=fAtarget=fZtarget=0; fEbeam=target_thickness=0;
   ...
   while( next_char != '+' && f.good() && !f.eof()){
      ...
      else
      {
         ...
         if( line.BeginsWith("Target") ){
            ...
            Int_t nlay; Float_t angle;
            sscanf(line.Data(), "%d %f", &nlay, &angle);
            Char_t mat[10]; Float_t thick;
