...
{
	//Default ctor.
	fRegList = new KVList; GetListRegs()->SetName("List regs array");
	SetOrientation('C');
	SetName("KVRegArray");
...
//__________________________________________________________________________
		 ...
		 ULong_t Address, Long_t Step, Char_t Orientation, Char_t *Voies, KVRegister*Registre)
 {
	 ...
	 SetName(Name); SetLabel("Register array");
	 SetAddress(Address); SetChannels(Voies); SetStep(Step); SetColumns(Columns); SetLines(Lines);
