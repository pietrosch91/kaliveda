...
KVRegShared::KVRegShared()
{
   //Default ctor.
   fRegList = new KVList;
   GetListRegs()->SetName("List regs share");
...
void KVRegShared::SetContents(const Long64_t Value, const Long64_t reg_index,  const Long64_t)
{
   ...
   TIter next_reg(GetListRegs());
   KVRegister* reg = 0;
   Int_t first_bit = 0;
   Int_t id = 1;
   while ((reg = (KVRegister*)next_reg()) && id != reg_index) {
      id++;
      first_bit += reg->GetNumberBits();
   }
...
Long64_t KVRegShared::GetContents(const Long64_t id_groupe,  const Long64_t) const
{
   ...
   if (id_groupe) {
      ...
      TString total("");
      TIter nextReg(GetListRegs(), kIterBackward);
      KVRegister* reg = 0;
