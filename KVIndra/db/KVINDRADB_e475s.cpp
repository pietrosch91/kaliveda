						...
						while (sline.BeginsWith("<LIST> Rings=")){
							...
							while ( sline.BeginsWith("<LIST> Modules=") || sline.BeginsWith("<PARAMETER> Function=")){
								if (sline.BeginsWith("<LIST> Modules=")) {
									...
									parset->SetParameter(ff.GetNpar()+1,0.);	  parset->SetParamName(ff.GetNpar()+1,"xmin");
									parset->SetParameter(ff.GetNpar()+2,4096.); parset->SetParamName(ff.GetNpar()+2,"xmax");
      ...
      while (sline.BeginsWith("+KVCalibrator")){
			...
			while (sline.BeginsWith("<LIST> Type=")){
				...
				while (sline.BeginsWith("<LIST> KVDetector=")) {
					sline.ReplaceAll("<LIST> KVDetector=",""); calib_det = sline;	cout << calib_det << endl; 
					sline.ReadLine(fin);
					while (sline.BeginsWith("<LIST> KVACQParam=")) {
						sline.ReplaceAll("<LIST> KVACQParam=",""); calib_acq = sline;	cout << calib_acq << endl; 
						sline.ReadLine(fin);
						while (sline.BeginsWith("++<LIST> Runs=")){
							sline.ReplaceAll("++<LIST> Runs=",""); 	calib_runs = sline;	cout << calib_runs << endl;
							sline.ReadLine(fin);
