...
class KVCutList : public KVList{
	...
	Float_t fThetaVamos;
	KVCutList(){};
	virtual ~KVCutList(){ fThetaVamos=0.;};
	const Char_t *GetName() const { return GetCutListName(fThetaVamos); };
   static const Char_t *GetCutListName( Float_t th_vamos ){
	   return Form("%.1f",th_vamos);
   }
   void SetThetaVamos( Float_t th_vamos ){ fThetaVamos = th_vamos; }
   Float_t GetThetaVamos() const { return fThetaVamos; }
...
class KVBasicVAMOSFilter : public KVBase
{
   ...
   Bool_t IsInThetaVrange( Double_t thetav ){ return -2<=thetav && thetav<=2; }
   Bool_t IsInPhiVrange( Double_t phiv ){ return -2<=phiv && phiv<=2; }
   Bool_t IsInDeltaRange( Double_t delta ){ return 0.945<=delta && delta<=1.2; }
   ...
   using KVBase::Print;
   void Print( Option_t *opt = "" ){ fMainList.ls(); }
