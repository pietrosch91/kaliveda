/*
 *	-----------------------------------------------------------------
 *	SeDv
 *	Initial structure from Maurycy Rejmund (2006)
 *  Rewritten by Sylvain Leblond (stage L3 IPN 2010) and Paolo Napolitani
 *	-----------------------------------------------------------------
 */
#include "SeDv.h"
#include "KVDataSet.h"
#include <cmath>
#include <cstdlib>

using namespace std;

ClassImp(SeDv)

SeDv::SeDv(LogFile *Log, Short_t nr)
{
#	ifdef DEBUG
	cout << "SeDv::Constructor" << endl;
#	endif
	fReady=kFALSE;

	Char_t line[255];
	Int_t len=255;
	Int_t i,j,k;
	Char_t fname[20];

	fLogFile = Log;
	fSeDNum = nr;

	for(i=0;i<14;i++) fCounter[i] = 0;
	for(i=0;i<4;i++) for(j=0;j<5;j++) fCounter1[i][j] = 0;

	InitRaw();
	Zero();


	fNStrips = 3;
	cout << "Number of considered strips: " << fNStrips << endl;
	fLogFile->Log << "Number of considered strips: " << fNStrips << endl;
	/*
 	 *	Open and read "SeD#Ref.cal"
 	 */
	sprintf(fname,"SeD%1dRef.cal",fSeDNum);
	ifstream inf;
	if(!gDataSet->OpenDataSetFile(fname, inf)) {
		cout << "Could not open the Reference file SeDRef.cal !" << endl; return; }
	else
	{
		cout.setf(ios::showpoint);
		cout << "Reading " << fname << endl; fLogFile->Log << "Reading "<< fname << endl;
		/*
 		 *		Reading Q-threshold parameter
 		 */
		inf.getline(line,len);
		cout << line << endl; fLogFile->Log << line << endl;
		inf >> fQThresh[0]; inf >> fQThresh[1];
		cout <<"  "<< fQThresh[0] <<", "<< fQThresh[1] << endl; fLogFile->Log <<"  "<< fQThresh[0] <<", "<< fQThresh[1] << endl; 
		/*
 		 *		Reading X geometric transformation matrix(j*k), first and second iteration(i)
 		 */
		for(i=0;i<2;i++) 
		{
		  	for(j=0;j<2;j++) inf.getline(line,len);
			cout << line << endl; fLogFile->Log << line << endl;
			for(j=0;j<=ORDRE_DU_FIT;j++) for(k=0;k<=ORDRE_DU_FIT;k++)
		  	{
				inf >> fRecadrageX_SED[i][j][k];
				printf("  %13.8g",fRecadrageX_SED[i][j][k]);  if(k==4) cout << endl; 
			}
		}
		/*	
 		 *		Reading Y geometric transformation matrix(j*k), first and second iteration(i)
 		 */
		for(i=0;i<2;i++) 
		{
		  	for(j=0;j<2;j++) inf.getline(line,len);
			cout << line << endl;
			fLogFile->Log << line << endl;
			for(j=0;j<=ORDRE_DU_FIT;j++) for(k=0;k<=ORDRE_DU_FIT;k++)
		  	{
		  		inf >> fRecadrageY_SED[i][j][k];
				printf("  %13.8g",fRecadrageY_SED[i][j][k]);  if(k==4) cout << endl; 
		  	}
		}
	}
	inf.close();
	/*
 	 *	Open and read "SeD#.cal"
 	 */
	sprintf(fname,"SeD%1d.cal",fSeDNum);
	if(!gDataSet->OpenDataSetFile(fname,inf)) { 
		cout << "Could not open the calibration file " << fname << endl; return; }
	else
	{
		cout.setf(ios::showpoint);
		cout << "Reading " << fname << endl; fLogFile->Log << "Reading " << fname << endl;
		/*	
 		 *		Reading fE coefficients :
 		 */
		inf.getline(line,len);
		cout << line << endl; fLogFile->Log << line << endl;
		for(i=0;i<3;i++) for(j=0;j<2;j++) 
		{
			inf >> fECoef[i][j]; 
			cout <<"  "<< fECoef[i][j] <<" "; if(j==1) cout << endl; 
		} 
		/*	
 		 *		Reading parameters for Time TSED#-HF, 7 parameters Float_t, a,b,c,d,e,f,t0 :
 		 */
		for(j=0;j<2;j++) inf.getline(line,len);
		cout << line << endl; fLogFile->Log << line << endl;
		for(k=0;k<7;k++)
		{
		  	inf >> fTSED_HF_Coef[k];
			cout <<"  "<< fTSED_HF_Coef[k] <<" "; if(k>=5) cout << endl ;  
			if(k==5) {cout<<"  t0: ";  for(j=0;j<2;j++) inf.getline(line,len);}	// t0 is in a new line
		}	
		/*	
 		 *		Reading charge calibration of the 128 vertical strips giving the x coordinate :
 		 */
		for(j=0;j<2;j++) inf.getline(line,len);
		cout << line << endl; fLogFile->Log << line << endl;
		for(k=0;k<1;k++) for(i=0;i<128;i++) for(j=0;j<3;j++) inf >> fQCoef[i][k][j];
		/*	
 		 *		Reading charge calibration of the 48 horizontal strips giving the y coordinate :
 		 */
		for(j=0;j<2;j++) inf.getline(line,len);
		cout << line << endl; fLogFile->Log << line << endl;
		for(k=1;k<2;k++) for(i=0;i<48;i++) for(j=0;j<3;j++) inf >> fQCoef[i][k][j];
	}
	inf.close();
	fReady=kTRUE;
}
//___________________________________________________________________

SeDv::~SeDv(void)
{
#	ifdef DEBUG
	cout << "SeDv::Destructor" << endl;
#	endif
	PrintCounters();
}
//___________________________________________________________________

void SeDv::PrintCounters(void)
{
	Int_t i;
#	ifdef DEBUG
	cout << "SeDv::PrintCounters" << endl;
#	endif

	cout << endl;
	cout << "SeDv::PrintCounters" << endl;
	cout << "Called: " << fCounter[0] << endl;
	cout << "E1: " << fCounter[1] << endl;
	cout << "E2: " << fCounter[2] << endl;
	cout << "T1: " << fCounter[3] << endl;
	cout << "T2: " << fCounter[4] << endl;
	cout << "PresentWires: " << fCounter[5] << endl;
	cout << "STRIP Mult >= " << fNStrips << endl;
	for(i=0;i<4;i++) cout << fCounter1[i][0] << " ";
	cout << endl;
	cout << "PresentStrips: " << fCounter[6] << endl;
	cout << "X: Sequence Hiperbolique" << endl;
	for(i=0;i<2;i++) cout << fCounter1[i][1] << " ";
	cout << endl;
	cout << "X: Weighted Average" << endl;
	for(i=0;i<2;i++) cout << fCounter1[i][2] << " ";
	cout << endl;
	for(i=0;i<2;i++) cout << fCounter1[i][3] << " ";
	cout << endl;
	cout << "PresentSubseqX: " << fCounter[7] << endl;
	cout << "PresentSubseqY: " << fCounter[8] << endl;
	cout << "Present: " << fCounter[9] << endl;

	fLogFile->Log << endl;
	fLogFile->Log << "SeDv::PrintCounters" << endl;
	fLogFile->Log << "Called: " << fCounter[0] << endl;
	fLogFile->Log << "E1: " << fCounter[1] << endl;
	fLogFile->Log << "E2: " << fCounter[2] << endl;
	fLogFile->Log << "T1: " << fCounter[3] << endl;
	fLogFile->Log << "T2: " << fCounter[4] << endl;
	fLogFile->Log << "PresentWires: " << fCounter[5] << endl;
	fLogFile->Log << "STRIP Mult >= " << fNStrips << endl;
	for(i=0;i<2;i++) fLogFile->Log << fCounter1[i][0] << " ";
	fLogFile->Log << endl;
	fLogFile->Log << "PresentStrips: " << fCounter[6] << endl;
	fLogFile->Log << "X: Sequence Hiperbolique" << endl;
	for(i=0;i<2;i++) fLogFile->Log << fCounter1[i][1] << " ";
	fLogFile->Log << endl;
	fLogFile->Log << "X: Weighted Average" << endl;
	for(i=0;i<2;i++) fLogFile->Log << fCounter1[i][2] << " ";
	fLogFile->Log << endl;
	for(i=0;i<2;i++) fLogFile->Log << fCounter1[i][3] << " ";
	fLogFile->Log << endl;
	fLogFile->Log << "PresentSubseqX: " << fCounter[7] << endl;
	fLogFile->Log << "PresentSubseqY: " << fCounter[8] << endl;
	fLogFile->Log << "Present: " << fCounter[9] << endl;
}
//___________________________________________________________________

void SeDv::Zero(void)
{
#	ifdef DEBUG
	cout << "SeDv::Zero" << endl;
#	endif
	Init();
	for(Int_t j=0;j<2;j++) for(Int_t i=0;i<128;i++)
	{
		fQ[i][j] = 0.0;
		fN[i][j] = 0 ;
	}
}
//___________________________________________________________________

void SeDv::InitRaw(void)
{
#	ifdef DEBUG
	cout << "SeDv::InitRaw" << endl;
#	endif

	for(Int_t i=0;i<3;i++) fE_Raw[i] = 0;
	fT_Raw[0] = 0;

	for(Int_t i=0;i<128;i++) for(Int_t j=0;j<2;j++)
	{
		fQ_Raw[j*128+i] = 0;
		fQ_Raw_Nr[j*128+i] = 0;
	}

	for(Int_t j=0;j<2;j++) fQ_RawM[j] = 0;
}
//___________________________________________________________________

void SeDv::Init(void)
{
#	ifdef DEBUG
	cout << "SeDv::Init" << endl;
#	endif
	fPresent = fPresentWires = fPresentStrips = false; 

	fE[0] = fE[1] = fE[2] = fT[0] = 0.0;
	fMult[0] = fMult[1] = 0;
	fX[0]   = fX[1]   = -500.0;
	fXS[0]  = fXS[1]  = -500.0;
	fXWA[0] = fXWA[1] = -500.0;
}
//___________________________________________________________________

void SeDv::Calibrate(void)
{
#	ifdef DEBUG
	cout << "SeDv::Calibrate" << endl;
#	endif

	Random Rnd;
	Int_t i,j,k;
	Float_t QTmp,QTmp_array[2][128], noise_mean[2],noise_variance[2],noise_stdeviation_[2],noise_stdeviation,noise_sum;
	Float_t max_mean[2],max_variance[2],max_stdeviation,max_sum;
	UShort_t Q_max_Nr[2];
	Float_t noise_threshold[2],Q_max[2]; 
	Bool_t Accepted_event; //true 
	/*
 	 *	Initialisations
 	 */	
	for(i=0;i<2;i++) for(j=0;j<128;j++) QTmp_array[i][j]=0;

	for(i=0;i<2;i++) 
	{
		Q_max[i] = noise_mean[i] = noise_variance[i] = noise_sum = noise_stdeviation_[i] = noise_threshold[i] = 0.;
		max_mean[i] = max_variance[i] = max_sum = 0.;
		Q_max_Nr[i] = 1000 ;
		Accepted_event = false;
	}

	for(i=0;i<3;i++) if(fE_Raw[i]>0)
	{
		Rnd.Next();
		for(j=0;j<2;j++) fE[i]+=powf((Float_t)fE_Raw[i]+Rnd.Value(),(Float_t)j)*fECoef[i][j];
	}
	if(fT_Raw[0]>0)
	{
		Rnd.Next();

		Double_t x,a,b,c,d,e,f,t0,m_run;
		x = (Double_t)fT_Raw[0]+(Double_t)Rnd.Value() ;
		a = (Double_t)fTSED_HF_Coef[0] ;
		b = (Double_t)fTSED_HF_Coef[1] ;
		c = (Double_t)fTSED_HF_Coef[2] ;
		d = (Double_t)fTSED_HF_Coef[3] ;
		e = (Double_t)fTSED_HF_Coef[4] ;
		f = (Double_t)fTSED_HF_Coef[5] ;
		t0= (Double_t)fTSED_HF_Coef[6] ;

		/*
 		 *		README README README README README README README README README README  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 		 *		
 		 *		PRELIMINARY CORRECTION: to change later 
 		 *		this is approximatively valid for the calibration runs 41-46
 		 *		this should be done more properly by verifying t0 in the parameters from file 
 		 *		and a multiplication factor m_run as a function of the run.
 		 *		one should perhaps write
 		 *		
 		 *		fT[0] = (Float_t)( -(a+(b+(c+(d+(e+f*x)*x)*x)*x)*x)*x * m_run + t0 );
 		 *		
 		 *		README README README README README README README README README README  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 		 */		
		m_run = 1./2.5 ;

		fT[0] = (Float_t)( -(a+(b+(c+(d+(e+f*x)*x)*x)*x)*x)*x*m_run+t0 );

#	ifdef DEBUG
		Info("SeDv::Calibrate","Traw %d [ch], TSED_HF %f [ns]",fT_Raw[0], fT[0]);
#	endif
 
	}

	if(fE[0] > 0.0) fCounter[1]++;
	if(fE[1] > 0.0) fCounter[2]++;
	if(fE[2] > 0.0) fCounter[3]++;
	if(fT[0] > 0.0) fCounter[4]++;

	if((fE[0] > 0.0 || fE[1] > 0.0 || fE[2] > 0.0) && fT_Raw[0] > 0.0)
	{
		fPresentWires = true;
		fCounter[5]++;
	}
	else 
	{
		fE[0] = fE[1] = fE[2] = 0.0;
		/*
 		 *		PRELIMINARY CORRECTION: to change later by the use of t0 
 		 *		if T[] is negative than it should be rejected 
 		 */		
		//		T[0] = 0.0;
	}
	/*
 	 *	Calculation of the threshold
 	 */
	if(!fPresentWires)
	{	
		for(i=0;i<2;i++)			//0) vertical, giving x. 1) vertical, giving y
		{
			/*
 			 *			Study the noise and the maximum value for the charge.
 			 *			At this stage the quantities noise_*** contain also the signal!
 			 */
			for(j=0;j<fQ_RawM[i];j++)	//loop over strips
			{
				Rnd.Next();
				for(k=0;k<3;k++)		//loop over coeff
					QTmp_array[i][j] += powf((Float_t)fQ_Raw[j+i*128]+Rnd.Value(),(Float_t)k)
						*fQCoef[fQ_Raw_Nr[j+i*128]][i][k];

				QTmp = QTmp_array[i][j];			
				if(QTmp >0)
				{
					noise_mean[i] += QTmp;
					noise_variance[i] += QTmp*QTmp;
					noise_sum += 1.;
				}
				if(QTmp > Q_max[i])	
				{
					Q_max[i] = QTmp;
					Q_max_Nr[i] = fQ_Raw_Nr[j+i*128];
				}
			}
/*
 *			Study the surrounding of the maximum charge
 */			
			for(j=-Border;j<=Border;j++)
			{
				k = Q_max_Nr[i]+j; if(k<=48+i*128 && k>=0) 
				{
					QTmp=QTmp_array[i][k];  
					if (QTmp>0)
					{
						max_mean[i] += QTmp;
						max_variance[i] += QTmp*QTmp;
						max_sum += 1.;	
					}
				}
			}
			/*
 			 *			The signal of the maximum charge is removed from the quantities noise_***
 			 */		
			noise_mean[i] -= max_mean[i];
			noise_variance[i] -= max_variance[i];
			noise_sum -= max_sum;
			/*
 			 *			Calculate statistical quntities related to the noise and fix the threshold
 			 *			as the mean of the noise + 1 time its standart deviation
 			 */			
			if(noise_sum<2. || noise_variance[i]==0) 
			{
				noise_mean[i] = noise_variance[i] =0;
			}
			else 
			{
				noise_mean[i] /= noise_sum;
				noise_variance[i] = noise_variance[i]/noise_sum - noise_mean[i]*noise_mean[i];
				noise_stdeviation_[i] = sqrt(noise_variance[i]);
			}
			noise_threshold[i] = noise_mean[i] + noise_stdeviation_[i];
			/*
 			 *			Calculate statistical quntities related to the maximum
 			 */			
			if(max_sum<2.) 
			{
				max_mean[i] = max_variance[i] =0;
			}
			else 
			{
				max_mean[i] /= max_sum;
				max_variance[i] = max_variance[i]/max_sum - max_mean[i]*max_mean[i];
			}
		}
		max_stdeviation = sqrt(max_variance[0] + max_variance[1]); 
		noise_stdeviation = sqrt(noise_variance[0] + noise_variance[1]); 
		if((noise_sum>3. && noise_stdeviation<2.*max_stdeviation) || (noise_sum<2. && max_sum>2.))
			Accepted_event= true ;
 	}
	/*
 	 *	Select the wires giving a signal above the threshold 
 	 */
	if(!fPresentWires)
	{	
		if(Accepted_event)
			for(i=0;i<2;i++)			//0) vertical, giving x. 1) vertical, giving y
				for(j=0;j<fQ_RawM[i];j++)	//loop over strips
				{
					QTmp=QTmp_array[i][j];
					if(QTmp > noise_threshold[i])
					{
						fMult[i]++;
						fQ[fMult[i]-1][i] = QTmp;		
						fN[fMult[i]-1][i] = fQ_Raw_Nr[j+i*128];		
					}
				}
		for(i=0;i<2;i++) if(fMult[i] >= fNStrips) fCounter1[i][0]++;

		if(fMult[0] >= fNStrips && fMult[1] >= fNStrips)
		{
			fPresentStrips = true;
			fCounter[6]++;
		}

#	ifdef DEBUG
		Info("SeDv::Calibrate","Event %s",  Accepted_event ? "accepted" : "rejected");  
		cout<<" maximum charge x,y      : " << Q_max[0]             <<" "<< Q_max[1]             <<endl;  
		cout<<" maximum charge wire x,y : " << Q_max_Nr[0]          <<" "<< Q_max_Nr[1]          <<endl;  
		cout<<" noise mean x,y          : " << noise_mean[0]        <<" "<< noise_mean[1]        <<endl;  
		cout<<" noise variance x,y      : " << noise_variance[0]    <<" "<< noise_variance[1]    <<endl;  
		cout<<" noise stdeviation x,y   : " << noise_stdeviation_[0]<<" "<< noise_stdeviation_[1]<<endl;  
		cout<<" noise threshold x,y     : " << noise_threshold[0]   <<" "<< noise_threshold[1]   <<endl;  
		cout<<" max mean x,y            : " << max_mean[0]          <<" "<< max_mean[1]          <<endl;  
		cout<<" max variance x,y        : " << max_variance[0]      <<" "<< max_variance[1]      <<endl;  
		cout<<" max stdeviation         : " << max_stdeviation   <<endl;  
#	endif
	}
}
//___________________________________________________________________

void SeDv::FocalSubseqX(void)
{
	Int_t i,j,k,m;
	Float_t QTmp[128];
	Float_t QMax;
	Int_t NMax;
	Int_t FStrip[128];
	Int_t StripsWA;
	Float_t v[6];
	Double_t XYp,stk1,stk0;

#	ifdef DEBUG
	cout << "SeDv::FocalSubseqX" << endl;
#	endif

	for(i=0;i<2;i++)							// 0) vertical, giving x. 1) vertical, giving y
	{
		/*
 		 *		Create an array of touched strips of dimension fMult[i]
 		 */
		for(j=0;j<fMult[i];j++) QTmp[j]=fQ[j][i];
		/*
 		 *		searching for a number NStrips of Strips, create an array of labels which order the strips 
 		 *		according to decreasing charge
 		 */
		for(k=0;k<fNStrips;k++)
		{
			QMax=0.0; NMax=0;
			for(j=0;j<fMult[i];j++) if(QTmp[j] > QMax)
			{ 
				QMax=QTmp[j]; 
				NMax=j; 
			}
			QTmp[NMax] = 0.0;
			FStrip[k] = NMax;
		} 
		/*
 		 *		Just a control
 		 */       
		if(fNStrips != 3) 
		{
			cout << "NStrips != 3 but " << fNStrips << " Program it first" << endl;
			exit(EXIT_FAILURE);
		}
		/*
 		 *		Looking for entire peak for weighted average. The Strips are ordered  0-128
 		 */
		if(fMult[i] > fNStrips)	
		{
			StripsWA=0;
			for(j=FStrip[0]-1;j>=0;j--) if(abs(fN[j+1][i]-fN[j][i])==1)
			{
				if((fQ[j][i] <= fQ[j+1][i]))
				{
					StripsWA++;
					FStrip[StripsWA]=j;
				}
				else break;
			}
			else break;

			for(j=FStrip[0];j<fMult[i]-1;j++) if(abs(fN[j+1][i]-fN[j][i])==1)
			{
				if(fQ[j][i] >= fQ[j+1][i])
				{
					StripsWA++;
					FStrip[StripsWA]=j+1;			  
				}
				else break;
			}
			else break;

			if(StripsWA >= fNStrips)
			{
				v[0] = v[1] = 0.0;
				for(k=0;k<=StripsWA;k++)
				{		      
					v[0] += fQ[FStrip[k]][i]* ((Float_t) fN[FStrip[k]][i]);
					v[1] += fQ[FStrip[k]][i];
				}
				fX[i] = v[0] / v[1];
			}		
		}
	}

#	ifdef DEBUG
	Info("SeDv::FocalSubseqX","(X,Y)= (%f,%f) [ch]",fX[0],fX[1]);
#	endif

/*
 *	Recadrage. On utilise les fonctions de corrections pour recadrer l'image. Passage en cm on the SeD plane
 */
	fX[0]+=1.; fX[1]+=1.; // To have wires starting at 1

	if(fX[0]>0 && fX[1]>0) for(i=0;i<2;i++)
	{
		/*
 		 *		Correction en X 1
 		 *		Abscissa calibration on the SeD# plane in cm
 		 */
		stk0 = (Double_t)fX[0];
		stk1 = (Double_t)fX[1];
		fX[0] = 0.;
		for(j=0;j<=ORDRE_DU_FIT;j++) for(k=0;k<=ORDRE_DU_FIT;k++) if(fRecadrageX_SED[i][j][k]!=0) 
		{
			XYp=1.; 
			for(m=0;m<j;m++) XYp*=stk0; 
			for(m=0;m<k;m++) XYp*=stk1;
			fX[0] += (Float_t)(fRecadrageX_SED[i][j][k] * XYp);
		}
		//		cout << "  Wire corrected x : "<< X[0]<< " "  << fX[1]<<  endl;
		/*
 		 *		Correction en Y 1
 		 *		Ordinate calibration on the SeD# plane in cm
 		 */
		stk0 = (Double_t)fX[0];
		stk1 = (Double_t)fX[1];
		fX[1] = 0.;
		for(j=0;j<=ORDRE_DU_FIT;j++) for(k=0;k<=ORDRE_DU_FIT;k++) if(fRecadrageY_SED[i][j][k]!=0) 
		{
			XYp=1.; 
			for(m=0;m<j;m++) XYp*=stk1; 
			for(m=0;m<k;m++) XYp*=stk0;
			fX[1] += (Float_t)(fRecadrageY_SED[i][j][k]* XYp);
		}
		//		cout << "  Wire corrected y : "<< fX[0]<< " "  << fX[1]<<  endl;
	}
	/*
 	 *	Passage cm -> mm on the projection of SeD#
 	 */
	fX[0] *= 10.;
	fX[1] *= 10.*ONEOVERSQRTTWO;
	/*
 	 *	Counters
 	 */
	fXWA[0]=fX[0];	fXWA[1]=fX[1];
	if ((fX[0]<Borneminx_SED || fX[0] >Bornemaxx_SED) ||
			(fX[1]<Borneminy_SED || fX[1] >Bornemaxy_SED))
		fXWA[0] = fX[0] = fXWA[1] = fX[1] = -500.;
	else fCounter1[i][2]++;

#	ifdef DEBUG
	Info("SeDv::FocalSubseqX","(X,Y)= (%f,%f) [mm]",fX[0],fX[1]);
#	endif


	if (fX[0]>Borneminx_SED && fX[0] <Bornemaxx_SED)	fCounter[7]++;
	if (fX[1]>Borneminy_SED && fX[1] <Bornemaxy_SED)	fCounter[8]++;

	if ((fX[0]>Borneminx_SED && fX[0] <Bornemaxx_SED)&&
	    	(fX[1]>Borneminy_SED && fX[1] <Bornemaxy_SED))
	{
		fPresent = true;
		fCounter[9]++;
	}
	else for(i=0;i<2;i++) fX[i] = -500.0;
}
//___________________________________________________________________

void SeDv::Treat(void)
{
#	ifdef DEBUG
	cout << "SeDv::Treat" << endl;
#	endif

	if(!fReady) return;
	fCounter[0]++;
	Init();
	Calibrate();
	if(fPresentStrips) FocalSubseqX();		//PresentWires by definition

#	ifdef DEBUG
	Show_Raw();
	Show();
#	endif
}
//___________________________________________________________________

void SeDv::inAttach(TTree *inT)
{
	Char_t strnam[20];

#	ifdef DEBUG
	cout << "SeDv::inAttach " << endl;
	cout << "Attaching Sed variables" << endl;
#	endif

	/* for(Int_t i=0;i<3;i++)
	   {
	   sprintf(strnam,"QSED%1d_%1d",fSeDNum,i+1);
	   inT->SetBranchAddress(strnam,&fE_Raw[i]);
	// }*/
	sprintf(strnam,"TSED%1d_HF",fSeDNum);
	inT->SetBranchAddress(strnam,&fT_Raw[0]);
	sprintf(strnam,"SEDMX%1d",fSeDNum);
	inT->SetBranchAddress(strnam,fQ_RawM+0);
	sprintf(strnam,"SEDMY%1d",fSeDNum);
	inT->SetBranchAddress(strnam,fQ_RawM+1);
	sprintf(strnam,"SEDX%1d",fSeDNum);
	inT->SetBranchAddress(strnam,fQ_Raw+0*128);
	sprintf(strnam,"SEDY%1d",fSeDNum);
	inT->SetBranchAddress(strnam,fQ_Raw+1*128);
	sprintf(strnam,"SEDNrX%1d",fSeDNum);
	inT->SetBranchAddress(strnam,fQ_Raw_Nr+0*128);
	sprintf(strnam,"SEDNrY%1d",fSeDNum);
	inT->SetBranchAddress(strnam,fQ_Raw_Nr+1*128);
}
//___________________________________________________________________

void SeDv::outAttach(TTree *outT)
{
	Char_t strnam[20];
	Char_t strnam1[20];
	Int_t i;

#	ifdef DEBUG
	cout << "SeDv::outAttach " << endl;
	cout << "Attaching SeD variables" << endl;
#	endif

	for(i=0;i<3;i++)
	{
		sprintf(strnam,"SeDEWire%1d_%1d",fSeDNum,i);
		sprintf(strnam1,"SeDEWire%1d_%1d/F",fSeDNum,i);
		outT->Branch(strnam,&fE[i],strnam1);
	}
	sprintf(strnam,"SeDTWire%1d",fSeDNum);
	sprintf(strnam1,"SeDTWire%1d/F",fSeDNum);
	outT->Branch(strnam,&fT[0],strnam1);

	sprintf(strnam,"SeDX%1d",fSeDNum);
	sprintf(strnam1,"SeDX%1d/F",fSeDNum);
	outT->Branch(strnam,&fX[0],strnam1);

	sprintf(strnam,"SeDY%1d",fSeDNum);
	sprintf(strnam1,"SeDY%1d/F",fSeDNum);
	outT->Branch(strnam,&fX[1],strnam1);

	sprintf(strnam,"SeDXS%1d",fSeDNum);
	sprintf(strnam1,"SeDXS%1d/F",fSeDNum);
	outT->Branch(strnam,&fXS[0],strnam1);

	sprintf(strnam,"SeDYS%1d",fSeDNum);
	sprintf(strnam1,"SeDYS%1d/F",fSeDNum);
	outT->Branch(strnam,&fXS[1],strnam1);

	sprintf(strnam,"SeDXWA%1d",fSeDNum);
	sprintf(strnam1,"SeDXWA%1d/F",fSeDNum);
	outT->Branch(strnam,&fXWA[0],strnam1);

	sprintf(strnam,"SeDYWA%1d",fSeDNum);
	sprintf(strnam1,"SeDYWA%1d/F",fSeDNum);
	outT->Branch(strnam,&fXWA[1],strnam1);
}
//___________________________________________________________________

void SeDv::CreateHistograms(void)
{
#	ifdef DEBUG
	cout << "SeDv::CreateHistograms " << endl;
#	endif
}
//___________________________________________________________________

void SeDv::FillHistograms(void)
{
#	ifdef DEBUG
	cout << "SeDv::FillHistograms " << endl;
#	endif
}
//___________________________________________________________________

void SeDv::Show_Raw(void)
{
#	ifdef DEBUG
	cout << "SeDv::Show_Raw" << endl;
#	endif
	cout.setf(ios::showpoint);

	cout << "E1_Raw: " << fE_Raw[0] << " E2_Raw: " << fE_Raw[1] << " E3_Raw: " << fE_Raw[2] <<endl;
	cout << "T1_Raw: " << fT_Raw[0] << endl;

//	for(Int_t i=0;i<2;i++)			//0) vertical, giving x. 1) vertical, giving y
//		for(Int_t j=0;j<fQ_RawM[i];j++)	//loop over strips
//			cout << "STR: " << fQ_Raw_Nr[j+i*128] << " SED: " << i << " Q_Raw: " <<  fQ_Raw[j+i*128] << endl;
}
//___________________________________________________________________

void SeDv::Show(void)
{
#	ifdef DEBUG
	cout << "SeDv::Show" << endl;
#	endif
	cout.setf(ios::showpoint); 

	cout << "PresentWires: " << fPresentWires << endl;
	if(fPresentWires)
	{
		cout << "E1: " << fE[0] << " E2: " << fE[1] << " E3: " << fE[2] <<endl;
		cout << "T1: " << fT[0] << endl;
	}
	cout << "PresentStrips: " << fPresentStrips << endl;
	if(fPresentStrips) for(Int_t i=0;i<2;i++)						//0) vertical, giving x. 1) vertical, giving y
	{
		cout << " Mult " << i << ": " << fMult[i] << endl;
//		for(j=0;j<fMult[i];j++)								//loop over strips touched
//			cout << "STR: " << fN[j][i] << " SED: " << i << " Q: " <<  fQ[j][i] << endl;
	}
	cout << "Xi: ";
	for(Int_t i=0;i<2;i++) cout << fX[i] << " ";					//0) vertical, giving x. 1) vertical, giving y		
	cout << endl;
	cout << "Present: " << fPresent << endl;
}
