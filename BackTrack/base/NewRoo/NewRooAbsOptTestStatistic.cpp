...
ClassImp(NewRooAbsOptTestStatistic)

////////////////////////////////////////////////////////////////////////////////
// BEGIN_HTML <!--
/* -->
<h2>NewRooAbsTestStatistic</h2>
<h4> Derived class of RooAbsTestStatistic to avoid RooArgSet checkForDup() messages</h4>
<!-- */
// --> END_HTML
////////////////////////////////////////////////////////////////////////////////
//_____________________________________________________________________________
NewRooAbsOptTestStatistic:: NewRooAbsOptTestStatistic()
{
  // Default Constructor
...
//_____________________________________________________________________________
NewRooAbsOptTestStatistic::NewRooAbsOptTestStatistic(const char *name, const char *title, RooAbsReal& real, RooAbsData& indata,
...
//_____________________________________________________________________________
NewRooAbsOptTestStatistic::NewRooAbsOptTestStatistic(const NewRooAbsOptTestStatistic& other, const char* name) : 
...
//_____________________________________________________________________________
void NewRooAbsOptTestStatistic::initSlave(RooAbsReal& real, NewRooAbsData& indata, const RooArgSet& projDeps, const char* rangeName, 
				       const char* addCoefRangeName) 
{
  ...
  while((arg=iter.next())) {
   ...
   if (!realReal->getBinning().lowBoundFunc() && realReal->getMin()<(datReal->getMin()-1e-6)) {
      coutE(InputArguments) << "NewRooAbsOptTestStatistic: ERROR minimum of FUNC observable " << arg->GetName() 
    ...
    if (!realReal->getBinning().highBoundFunc() && realReal->getMax()>(datReal->getMax()+1e-6)) {
      coutE(InputArguments) << "NewRooAbsOptTestStatistic: ERROR maximum of FUNC observable " << arg->GetName() 
...
//     cout << "NewRooAbsOptTestStatistic: reducing dataset to fit in range named " << rangeName << " resulting dataset has " << _dataClone->sumEntries() << " events" << endl ;
  } else {
  ...
  if (rangeName && strlen(rangeName)) {    
    cxcoutI(Fitting) << "NewRooAbsOptTestStatistic::ctor(" << GetName() << ") constructing test statistic for sub-range named " << rangeName << endl ;    
...
//  	     << realObs->GetName() << " to [" << realObs->getMin() << "," << realObs->getMax() << "]" << endl ;
  ...
  if (rangeName && strlen(rangeName)) {
    ...
    if (addCoefRangeName && strlen(addCoefRangeName)) {
      cxcoutI(Fitting) << "NewRooAbsOptTestStatistic::ctor(" << GetName() 
...
//     delete tobedel ;
  ...
  coutI(Optimization) << "NewRooAbsOptTestStatistic::ctor(" << GetName() << ") optimizing internal clone of p.d.f for likelihood evaluation." 
...
//_____________________________________________________________________________
Double_t NewRooAbsOptTestStatistic::combinedValue(RooAbsReal** array, Int_t n) const
{
  ...
  for (Int_t i = 0; i < n; ++i) {
    ...
    carry += reinterpret_cast<NewRooAbsOptTestStatistic*>(array[i])->getCarry();
    y -= carry;
...
//_____________________________________________________________________________
Bool_t NewRooAbsOptTestStatistic::redirectServersHook(const RooAbsCollection& newServerList, Bool_t mustReplaceAll, Bool_t nameChange, Bool_t isRecursive) 
{
...
//_____________________________________________________________________________
void NewRooAbsOptTestStatistic::printCompactTreeHook(ostream& os, const char* indent) 
{
...
//_____________________________________________________________________________
void NewRooAbsOptTestStatistic::constOptimizeTestStatistic(ConstOpCode opcode, Bool_t doAlsoTrackingOpt) 
{
  ...
  if (_dataClone->hasFilledCache() && _dataClone->store()->cacheOwner()!=this) {
    if (opcode==Activate) {
      cxcoutW(Optimization) << "NewRooAbsOptTestStatistic::constOptimize(" << GetName() 
  ...
  if (!allowFunctionCache()) {
    if (opcode==Activate) {
      cxcoutI(Optimization) << "NewRooAbsOptTestStatistic::constOptimize(" << GetName() 
...
//_____________________________________________________________________________
void NewRooAbsOptTestStatistic::optimizeCaching() 
{
...
//   cout << "NewRooAbsOptTestStatistic::optimizeCaching(" << GetName() << "," << this << ")" << endl ;
...
//_____________________________________________________________________________
void NewRooAbsOptTestStatistic::optimizeConstantTerms(Bool_t activate, Bool_t applyTrackingOpt)
{
  ...
  if(activate) {
	...
	coutW(Optimization) << "NewRooAbsOptTestStatistic::optimizeConstantTerms(" << GetName() 
...
//_____________________________________________________________________________
Bool_t NewRooAbsOptTestStatistic::setDataSlave(RooAbsData& indata, Bool_t cloneData, Bool_t ownNewData) 
{ 
//   cout << "RAOTS::setDataSlave(" << this << ") START" << endl ;
  ...
  if (!cloneData && _rangeName.size()>0) {
    coutW(InputArguments) << "NewRooAbsOptTestStatistic::setData(" << GetName() << ") WARNING: test statistic was constructed with range selection on data, "
...
//_____________________________________________________________________________
RooAbsData& NewRooAbsOptTestStatistic::data() 
{ 
  if (_sealed) {
    Bool_t notice = (sealNotice() && strlen(sealNotice())) ;
    coutW(ObjectHandling) << "NewRooAbsOptTestStatistic::data(" << GetName() 
...
//_____________________________________________________________________________
const RooAbsData& NewRooAbsOptTestStatistic::data() const 
{ 
  if (_sealed) {
    Bool_t notice = (sealNotice() && strlen(sealNotice())) ;
    coutW(ObjectHandling) << "NewRooAbsOptTestStatistic::data(" << GetName() 
...
void NewRooAbsOptTestStatistic::enableSilentOffsetting(Bool_t flag) 
{
  // Apply internal value offsetting to control numeric precision
  // Modified method to have silence option when using initialize  
  ...
  case SimMaster:
    ...
    for (Int_t i = 0; i < _nGof; ++i) {
      //const_cast<NewRooAbsOptTestStatistic*>(_gofArray[i])->enableSilentOffsetting(flag);
  ...
  case MPMaster:    
    ...
    for (Int_t i = 0; i < _nCPU; ++i) {
      //const_cast<NewRooAbsOptTestStatistic*>(_mpfeArray[i])->enableSilentOffsetting(flag);
...
//_____________________________________________________________________________
Bool_t NewRooAbsOptTestStatistic::initializeSilent()
{
  ...
  //
  // Silence option for iniMPMode added 
  
  if (_init) return kFALSE;
  
  if (MPMaster == _gofOpMode) {
    initMPModeSilent(_func,_data,_projDeps,_rangeName.size()?_rangeName.c_str():0,_addCoefRangeName.size()?_addCoefRangeName.c_str():0) ;
...
void NewRooAbsOptTestStatistic::initMPModeSilent(RooAbsReal* real, RooAbsData* data, const RooArgSet* projDeps, const char* rangeName, const char* addCoefRangeName)
{
  ...
  for (Int_t i = 0; i < _nCPU; ++i) {
    ...
    ccoutD(Eval) << "NewRooAbsOptTestStatistic::initMPMode: starting remote server process #" << i << endl;
  ...
  _mpfeArray[_nCPU - 1]->addOwnedComponents(*gof, kTRUE);  //Modified
  coutI(Eval) << "NewRooAbsOptTestStatistic::initMPMode: started " << _nCPU << " remote server process." << endl;
