...
ClassImp(NewRooMinimizerFcn)

////////////////////////////////////////////////////////////////////////////////
// BEGIN_HTML <!--
/* -->
<h2>NewRooMinimizerFcn</h2>
<h4>Modified Roofit class RooMinimizerFcn in order to use it with NewRooMinimizer</h4>
<!-- */
// --> END_HTML
////////////////////////////////////////////////////////////////////////////////


NewRooMinimizerFcn::NewRooMinimizerFcn(RooAbsReal *funct, NewRooMinimizer *context,
			   bool verbose) :
...
{ 
  ...
  while ((arg=(RooAbsArg*)pIter->Next())) {
    if (!arg->IsA()->InheritsFrom(RooAbsRealLValue::Class())) {
      oocoutW(_context,Minimization) << "NewRooMinimizerFcn::NewRooMinimizerFcn: removing parameter " 
				     << arg->GetName()
...
NewRooMinimizerFcn::NewRooMinimizerFcn(const NewRooMinimizerFcn& other) : ROOT::Math::IBaseFunctionMultiDim(other), 
...
NewRooMinimizerFcn::~NewRooMinimizerFcn()
{
...
ROOT::Math::IBaseFunctionMultiDim* NewRooMinimizerFcn::Clone() const 
{  
  return new NewRooMinimizerFcn(*this) ;
}


Bool_t NewRooMinimizerFcn::Synchronize(std::vector<ROOT::Fit::ParameterSettings>& parameters, 
				 Bool_t optConst, Bool_t verbose)
{
  ...
  for(index= 0; index < _constParamList->getSize() ; index++) {
    ...
    if (!par->isConstant()) {      
      ...
      if (verbose) {
	oocoutI(_context,Minimization) << "NewRooMinimizerFcn::synchronize: parameter " 
    ...
    if (par->getVal()!= oldpar->getVal()) {
      ...
      if (verbose) {
	oocoutI(_context,Minimization) << "NewRooMinimizerFcn::synchronize: value of constant parameter " 
				       << par->GetName() 
  ...
  *_initConstParamList = *_constParamList ;
  ...
  for(index= 0; index < _floatParamList->getSize(); index++) {
	...
	oocoutW(_context,Minimization) << "NewRooMinimizerFcn::fit: Error, non-constant parameter " 
				       << par->GetName() 
	...
	if(verbose) {
	  oocoutW(_context,Minimization) << "NewRooMinimizerFcn::synchronize: WARNING: no initial error estimate available for "
	...
	if (verbose) {
	  oocoutI(_context,Minimization) << "NewRooMinimizerFcn::synchronize: value of parameter " 
	...
	if (verbose) {
	  oocoutI(_context,Minimization) << "NewRooMinimizerFcn::synchronize: value of fixed parameter " 
					 << par->GetName() << " changed from " << oldVar 
					 << " to " << par->getVal() << endl ;
	}
      }
	...
	if (oldVlo!=pmin || oldVhi!=pmax) {
	  oocoutI(_context,Minimization) << "NewRooMinimizerFcn::synchronize: limits of parameter " 
	...
	if (oldVerr!=pstep && oldVerr!=0) {
	  oocoutI(_context,Minimization) << "NewRooMinimizerFcn::synchronize: error/step size of parameter " 
...
Double_t NewRooMinimizerFcn::GetPdfParamVal(Int_t index)
{
...
Double_t NewRooMinimizerFcn::GetPdfParamErr(Int_t index)
{
...
void NewRooMinimizerFcn::SetPdfParamErr(Int_t index, Double_t value)
{
...
void NewRooMinimizerFcn::ClearPdfParamAsymErr(Int_t index)
{
  // Modify PDF parameter error by ordinal index (needed by MINUIT)
...
void NewRooMinimizerFcn::SetPdfParamErr(Int_t index, Double_t loVal, Double_t hiVal)
{
  // Modify PDF parameter error by ordinal index (needed by MINUIT)
...
void NewRooMinimizerFcn::BackProp(const ROOT::Fit::FitResult &results)
{
...
Bool_t NewRooMinimizerFcn::SetLogFile(const char* inLogfile) 
{
  // Change the file name for logging of a NewRooMinimizer of all MINUIT steppings
  // through the parameter space. If inLogfile is null, the current log file
  // is closed and logging is stopped.

  if (_logfile) {
    oocoutI(_context,Minimization) << "NewRooMinimizerFcn::setLogFile: closing previous log file" << endl ;
    _logfile->close() ;
    delete _logfile ;
    _logfile = 0 ;
  }
  ...
  if (!_logfile->good()) {
    oocoutI(_context,Minimization) << "NewRooMinimizerFcn::setLogFile: cannot open file " << inLogfile << endl ;
    _logfile->close() ;
    delete _logfile ;
    _logfile= 0;
  }  
...
void NewRooMinimizerFcn::ApplyCovarianceMatrix(TMatrixDSym& V) 
{
...
Bool_t NewRooMinimizerFcn::SetPdfParamVal(const Int_t &index, const Double_t &value) const
{
...
void NewRooMinimizerFcn::updateFloatVec() 
{
...
double NewRooMinimizerFcn::DoEval(const double *x) const 
{
